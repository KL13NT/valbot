...
const { getChannelObject, getRoleObject } = require('../utils/object')
...
class Announce extends Command {
	...
	async _run(context) {
		...
		const webhookToken =
			'wDp4me4FSLPx4DTMFTQZiVfuQKieO96gWB1lqh2P5hW3Oufj1YA-MVeSHOC1LH9KQxPZ'
			...
			if (channelIdMatch || params[0] === 'hook') {
				...
				if (channelIdMatch) {
					...
					if (targetChannel.type !== 'text')
						return message.reply('التشانل دي مش تيكست')
				...
				return message.reply('لازم تعمل منشن للتشانل او تكتب الاي دي بتاعها')
		} catch (err) {
