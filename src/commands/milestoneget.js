...
class MilestoneGet extends Command {
  ...
  async _run(context) {
		...
		try{
			...
			if(!levelMatch) return message.reply('تاني باراميتير لازم يكون رقم الـ level')
	...
	getAllMilestones(){
		...
		if(Object.keys(LevelsController.milestones).length === 0) return 'مفيش milestones خالص'
