...
const { CHANNEL_POLLS } = require('../config/config.js').CHANNELS
...
class Poll extends Command {
	...
	async _run(context) {
		...
		const pollsChannel = getChannelObject(this.client, CHANNEL_POLLS)
		...
		message.reply(
			'ابعت بقى رسالة فيها عنوان الاستفتاء. مش هتقدر تلغي الا ف الاخر خالص.'
		)
			...
			collector.on('collect', collected => {
				if (collected.member.id === member.id) {
					if (collected.content === 'cancel') {
						...
						message
							.reply(
								'تمام, دلوقتي بقى ريأكت على الرسالة دي بالريأكشنز اللي انت عايزها و لما تخلص اكتبلي اي حاجة وانا هفهم'
							)
	...
	async createPoll(poll) {
		if (poll.content === '' || poll.title === '')
			...
			const errorHandler = err => (
				console.log(err),
				message.reply(
					'في حاجة حصلت وانا بعمل الاستفتاء. جرب تاني او بص ف اللوجز'
				)
			)
			const reactByAll = sentPollMessage =>
				poll.reactions.forEach(reaction =>
					sentPollMessage.react(reaction).catch(errorHandler)
				)

			if (poll.content.length > 1000) {
				if (finalMessage.length > 2000)
					return message.reply(
						`احم... بص, الرسالة طويلة اوي و مش هعرف ابعتها. قصر الموضوع شوية و جرب تاني. محتاج تشيل ${
							finalMessage.length - 2000
						} حرف`
					)
